...
export const frontDeskAgentForPhone = (
  ...
  return new RealtimeAgent({
    ...
    6. Do not answer any questions that are not related to trip bookings or travel related questions or destination city weather.
    7. Today is ${new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}.
    8. You only serve hotel, car rental, and flight bookings.
    9. Speak English only. Do not use any other language.
    10. Currently we are testing this agent with a small number of customers. Please response as quick, fast as possible.
    ${mcpServers.length > 0 ? '11. You have access to tools through MCP server for searching hotels, car rentals, flights, getting weather information and canceling existing bookings etc.' : ''}
    `,
    tools: [
