...
const EndCallButton = React.memo(
  ({ onEnd }: { onEnd: () => void }): React.ReactElement => {
    return (
...
const ChatBot = (): React.ReactElement => {
  ...
  const processAudioQueue = useCallback(async () => {
    ...
    try {
      ...
      const resampledLength = Math.floor(
        combinedFloat32Array.length * resampleRatio
      )
      ...
      for (let i = 0; i < resampledLength; i++) {
        ...
        if (index1 >= combinedFloat32Array.length) {
          ...
          resampledArray[i] =
            combinedFloat32Array[index1] * (1 - fraction) +
  ...
  useEffect(() => {
    if (
      ...
      const audioChunkFromServer = websocketTunnel.responseData
        .data as ArrayBuffer
    ...
    <ChatContainer>
      ...
      <ChatInputContainer>
        {isSessionStartedRef.current ? (
          <EndCallButton onEnd={handleVoiceEnd} />
        ) : (
          <StartCallButton
